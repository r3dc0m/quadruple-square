...
function findBestBotMove(fullBoard, availablePositions, botCards, game) {
    ...
    const corners = [0, 2, 6, 8]; // corners
    const edges = [1, 3, 5, 7]; // edges
    const center = 4;
        ...
        for (const card of botCards) {
            ...
            if (captures > maxCaptures) {
                ...
                if (!wasCorner && willBeCorner) {
                    best = { position, cardId: card.card_id };
                } else if (!isCorner && isCenter && position === 4) {
                }
            }
        }
    }
    ...
    if (maxCaptures === 0) {
        const cornerSpaces = availablePositions
            .filter(pos => corners.includes(pos.position))
            .map(pos => ({
                position: pos.position,
                cardId: botCards[0].card_id
            }));

        if (cornerSpaces.length > 0) {
            const cornerPlay = cornerSpaces[0]; //frist corner
            return {
                position: cornerPlay.position,
                cardId: cornerPlay.cardId
            };
        }

        const edgeSpaces = availablePositions
            .filter(pos => edges.includes(pos.position))
            .map(pos => ({
                position: pos.position,
                cardId: botCards[0].card_id
            }));

        if (edgeSpaces.length > 0) {
            return edgeSpaces[0];
        }

        //any
        const anySpace = availablePositions[0];
        return {
            position: anySpace.position,
            cardId: botCards[0].card_id
        };
    }
